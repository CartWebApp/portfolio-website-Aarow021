...
export function getRootVar(propertyName) {
    ...
    for (const char of propertyName) {
        parsedName += char === '_' ? '-' : char;
    }
    if (window.getComputedStyle(document.documentElement).getPropertyValue('--' + parsedName) != '') {
        document.documentElement.style.setProperty('--' + parsedName, value);
    }
}
